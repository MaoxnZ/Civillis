...
Beyond the local bypass zone, skulls exert a **range-bounded attraction effect**. When a hostile mob attempts to spawn outside the local skull zone, that spawn can be **suppressed** based on:
...
The net effect: eligible distant spawns are probabilistically cancelled, so activity concentrates near your skulls — useful for farms without lighting every cave.
