...
/**
 * TTL-evicting civilization cache with gradual decay.
 * 
 * <p>Core features:
 * <ul>
 *   <li>Hot cache TTL: entries expire after 30 minutes to bound memory usage</li>
 *   <li>Cold storage: expired L2/L3 entries are persisted to H2 and restored on demand</li>
 *   <li>Gradual decay: civilization scores decay via {@code presenceTime} (ServerClock-based),
 *       not via hard TTL cutoff</li>
 *   <li>Conservative estimate: returns max civilization value when data is not yet loaded</li>
 * </ul>
 */
public final class TtlPyramidCache implements CivilizationCache {
    ...
    public TtlPyramidCache(long ttlMillis) {
    ...
    public Optional<CScore> getChunkCScore(ServerWorld level, VoxelChunkKey key) {
        ...
        if (entry != null) {
            // Check TTL (memory management, not civilization decay)
    ...
    /**
     * Touch an existing L2 entry's presenceTime for player-presence-driven recovery.
     * Called from the prefetcher when a player is near this L2 block.
     * No-op if the entry is not in hot cache or has expired.
     */
    public void visitL2(ServerWorld level, L2Key key) {
        String k = l2Key(level, key);
        TimestampedEntry<L2Entry> entry = l2Cache.get(k);
        if (entry != null && !entry.isExpired(ttlMillis)) {
            entry.getValue().onVisit(ServerClock.now());
        }
    }
     ...
     *
     * <p>Note: presenceTime recovery is driven by player proximity via
     * {@link #visitL2(ServerWorld, L2Key)}, called from the prefetcher.
     * Block-change-triggered L1 recomputation does NOT advance presenceTime,
     * because natural block changes (crop growth, etc.) are not player visits.
     */
     ...
     *
     * <p>Always restores the entry (no hard TTL cutoff). Civilization decay is now
     * handled by the gradual decay system via {@link L2Entry#computeDecayFactor(long)}.
     *
     * @param createTime timestamp from cold storage (wall-clock, used for hot cache TTL)
     * @return always true (entry is always restored)
     */
    public boolean restoreL2(ServerWorld level, L2Key key, L2Entry l2Entry, long createTime) {
        String k = l2Key(level, key);
        // Restore with current wall-clock time (start new 30-minute hot cache countdown)
        l2Cache.put(k, new TimestampedEntry<>(l2Entry, System.currentTimeMillis()));

        if (CivilMod.DEBUG) {
            double decay = l2Entry.computeDecayFactor(ServerClock.now());
            CACHE_LOG.info("[civil-cache] L2 RESTORED dim={} key={},{},{} presenceTime={} decayFactor={}", 
                    level.getRegistryKey(), key.getC2x(), key.getC2z(), key.getS2y(),
                    l2Entry.getPresenceTime(), String.format("%.3f", decay));
        }
        return true;
    }
    ...
    /**
     * Touch an existing L3 entry's presenceTime for player-presence-driven recovery.
     * Called from the prefetcher when a player is near this L3 block.
     * No-op if the entry is not in hot cache or has expired.
     */
    public void visitL3(ServerWorld level, L3Key key) {
     ...
     *
     * <p>Note: presenceTime recovery is driven by player proximity via
     * {@link #visitL3(ServerWorld, L3Key)}, called from the prefetcher.
     */
     ...
     *
     * <p>Always restores the entry (no hard TTL cutoff). Civilization decay is now
     * handled by the gradual decay system via {@link L3Entry#computeDecayFactor(long)}.
     *
     * @param createTime timestamp from cold storage (wall-clock, used for hot cache TTL)
     * @return always true (entry is always restored)
     */
    public boolean restoreL3(ServerWorld level, L3Key key, L3Entry l3Entry, long createTime) {
        ...
        if (CivilMod.DEBUG) {
            double decay = l3Entry.computeDecayFactor(ServerClock.now());
            CACHE_LOG.info("[civil-cache] L3 RESTORED dim={} key={},{},{} presenceTime={} decayFactor={}", 
                    level.getRegistryKey(), key.getC3x(), key.getC3z(), key.getS3y(),
                    l3Entry.getPresenceTime(), String.format("%.3f", decay));
        }
        return true;
    }
