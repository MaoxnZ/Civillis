...
/**
 * L3 cache entry: stores detailed state and aggregated values of 243 L1 cells.
 *
...
public final class L3Entry {
    ...
    public L3Entry(L3Key key) {
        this(key, civil.civilization.ServerClock.now());
    }

    public L3Entry(L3Key key, long presenceTime) {
        this.key = key;
        this.scores = new double[L3Key.CELL_COUNT];
        this.states = new byte[L3Key.CELL_COUNT];
        this.scoreSum = 0;
        this.validCount = 0;
        this.dirtyCount = 0;
        this.dirty = false;
    ...
    /**
     * Calculate coarse score (for L3 contribution calculation).
     * 
     * @return scoreSum (accumulated value, range [0, 243]), equivalent to full scan accumulation result
     */
    ...
    public void forEachDirtyCell(java.util.function.IntConsumer consumer) {
        for (int i = 0; i < L3Key.CELL_COUNT; i++) {
    ...
    public void restoreFromArrays(double[] scoresArray, byte[] statesArray) {
        if (scoresArray.length != L3Key.CELL_COUNT || statesArray.length != L3Key.CELL_COUNT) {
            return;
        }
        System.arraycopy(scoresArray, 0, this.scores, 0, L3Key.CELL_COUNT);
        System.arraycopy(statesArray, 0, this.states, 0, L3Key.CELL_COUNT);
