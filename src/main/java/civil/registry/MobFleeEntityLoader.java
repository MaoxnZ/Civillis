...
/**
 * Datapack reload: loads {@code data/&lt;namespace&gt;/civil_mob_flee_entities/*.json} into
 * {@link MobFleeEntityRegistry}.
 *
 ...
 */
public final class MobFleeEntityLoader {

    private static final Logger LOGGER = LoggerFactory.getLogger("civil-registry");
    private static final String DATA_PATH = "civil_mob_flee_entities";

    private MobFleeEntityLoader() {
    }
    ...
    public static void reload(ResourceManager manager) {
        ...
        for (Map.Entry<Identifier, Resource> entry : sorted) {
            ...
            try (InputStream is = entry.getValue().open();
                ...
                if (replace) {
                    LOGGER.warn("[civil-registry] replace=true in {} — clearing mob flee blacklist={} whitelist={}",
                ...
                LOGGER.error("[civil-registry] Failed to load mob flee entities from {}: {}",
                        fileId, e.getMessage());
            }
        }
        ...
        for (EntityType<?> t : whitelist) {
            if (blacklist.contains(t)) {
                LOGGER.warn("[civil-registry] Entity {} appears in both mob flee blacklist and whitelist; whitelist wins",
                        BuiltInRegistries.ENTITY_TYPE.getKey(t));
            }
        }

        MobFleeEntityRegistry.reload(blacklist, whitelist);
        LOGGER.info("[civil-registry] Loaded mob flee entities: blacklist={} whitelist={}",
        ...
        for (JsonElement el : arr) {
            ...
            try {
                ...
                if (BuiltInRegistries.ENTITY_TYPE.containsKey(parsed)) {
                    ...
                    if (CivilMod.DEBUG) {
                        LOGGER.warn("[civil-registry] Unknown entity type '{}' in mob flee {} ({}), skipping",
                                idStr, fileId, key);
                    }
                }
            } catch (Exception ex) {
                if (CivilMod.DEBUG) {
                    LOGGER.warn("[civil-registry] Invalid entity id '{}' in mob flee {} ({}), skipping",
                            idStr, fileId, key);
                }
            }
        }
